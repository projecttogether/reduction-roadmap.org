...
initialize_repository() {
    ...
    git -C "$app_dir" fetch --depth=50 origin "$repository_branch" || return 1
...
if ! initialize_repository; then
    echo "WARNING: Git Content repository initialization failed; the website will start, but Git Content will be unavailable." >&2
fi
