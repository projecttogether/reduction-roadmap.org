...
<a href="#page" class="skip-link px-4 py-2 bg-dark-green text-off-white font-bold no-underline rounded-b">Skip to Content</a>

<header class="sticky top-0 z-50 bg-off-white border-b border-transparent" id="header">
  <div class="container flex items-center justify-between gap-8 py-4">

    <div>
      <a href="<?= $site->url() ?>" class="block no-underline leading-none" aria-label="<?= $site->title()->html() ?> – Home">
        <?php if ($logo = $site->logo()->toFile()): ?>
          <img src="<?= $logo->url() ?>" alt="<?= $site->title()->html() ?>" class="h-12 w-auto" width="<?= $logo->width() ?>" height="<?= $logo->height() ?>">
        <?php else: ?>
    ...
    <nav class="hidden md:block" aria-label="Main navigation">
      <ul class="flex gap-1 list-none m-0 p-0" role="list">
  ...
  >
    <ul class="list-none m-0 p-0 text-center" role="list">
      <?php foreach ($site->children()->listed() as $item): ?>
        <li>
          <a
            href="<?= $item->url() ?>"
