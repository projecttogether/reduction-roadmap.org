<section class="section section--white section--download-cta">
  <div class="container">
    <?php if ($block->bodyText()->isNotEmpty()): ?>
      <div class="download-cta__body"><?= $block->bodyText()->kt() ?></div>
    <?php endif ?>
    <?php if ($block->headline()->isNotEmpty()): ?>
      <h2 class="download-cta__headline"><?= $block->headline()->html() ?></h2>
    <?php endif ?>
    <?php if ($block->buttonText()->isNotEmpty() && $block->buttonUrl()->isNotEmpty()): ?>
      <a href="<?= $block->buttonUrl()->esc() ?>" class="btn btn--primary btn--large">
        <?= $block->buttonText()->html() ?>
      </a>
    <?php endif ?>
  </div>
</section>
